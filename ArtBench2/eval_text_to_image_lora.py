...
#         return {"pixel_values": pixel_values, "input_ids": input_ids}
    ...
    for index in range(args.start, args.end+1):
        print(index)
        model_path = '{}/sd-lora-sub-{}-{}'.format(args.output_dir, index, args.seed)
        print(model_path)
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            for index_t, t in enumerate(range(0, 1000, 1000//1000)):
                ...
                # loss = F.mse_loss(model_pred.float(), torch.zeros_like(model_pred).float(), reduction="none")
        ...
        # if "idx-val.pkl" in args.index_path:
        #     with open('{}/sd-lora-sub-{}-{}/e-1-t-1000-val.pkl'.format(args.output_dir, index, args.seed), 'wb') as handle:
        #         pickle.dump(batch_loss_list, handle)
        # else:
        #     with open('{}/sd-lora-sub-{}-{}/e-1-t-1000-gen.pkl'.format(args.output_dir, index, args.seed), 'wb') as handle:
        #         pickle.dump(batch_loss_list, handle)

        while True:
            try:
                if "idx-val.pkl" in args.index_path:
                    with open('{}/sd-lora-sub-{}-{}/e-{}-val.pkl'.format(args.output_dir, index, args.seed, args.e_seed), 'wb') as handle:
                        pickle.dump(batch_loss_list, handle)
                else:
                    with open('{}/sd-lora-sub-{}-{}/e-{}-gen.pkl'.format(args.output_dir, index, args.seed, args.e_seed), 'wb') as handle:
                        pickle.dump(batch_loss_list, handle)
