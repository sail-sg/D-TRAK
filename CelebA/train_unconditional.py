...
def main(args):
    ...
    dataset = DatasetDict({
        "train": Dataset.from_dict({
            "img": df.loc[sub_idx]['path'].tolist(),
    ...
    augmentations = transforms.Compose(
        [
            ####
            transforms.CenterCrop(140), # song yang
            ####
    ...
    print(math.ceil((len(train_dataloader) * args.num_epochs)*0.1))
    print(math.ceil((len(train_dataloader) * args.num_epochs)*0.05))
    # print(math.ceil((len(train_dataloader) * args.num_epochs)*0.01))

    args.lr_warmup_steps = math.ceil((len(train_dataloader) * args.num_epochs)*0.1)
    
    if args.checkpointing_steps==-1:
        args.checkpointing_steps=math.ceil((len(train_dataloader) * args.num_epochs)*0.05)
